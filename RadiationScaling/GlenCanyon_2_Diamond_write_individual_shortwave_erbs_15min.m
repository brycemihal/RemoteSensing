...
%% Interpolate ERA5 to 15 minute timesteps
eraFields = fieldnames(era5.selected);
era5_SolarTimes = era5.selected.(eraFields{1}).SolarRadiation{1};
era5_SolarTimes = era5_SolarTimes(1):minutes(15):era5_SolarTimes(end);
...
matlabDataFile = 'era5_with_radiation_factors_15min.mat';
if ~exist(matlabDataFile,'file')>0
    for i = 1:length(uInd)
        %% get solar data
        era5_Solar = era5.selected.(eraFields{i}).SolarRadiation{2}';
        era5_SolarTimesHR = era5.selected.(eraFields{i}).SolarRadiation{1};
        % interpolate to 15 minutes
        era5_SolarTimes = era5_SolarTimesHR(1):minutes(15):era5_SolarTimesHR(end);
        era5_Solar = interp1(era5_SolarTimesHR,era5_Solar,era5_SolarTimes);
